...
<div class="dashboard-container">
    ...
    <div class="recent-activity">
        ...
        <div class="activity-container">
            ...
            try {
                $stmt = $pdo->query("
                    SELECT t.*, b.title, b.accession_number, m.full_name, m.unique_identifier,
                           CASE 
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    while ($activity = $stmt->fetch()) {
                        $status_class = strtolower(str_replace(' ', '-', $activity['display_status']));
                        $icon = $activity['status'] == 'Issued' ? 'fa-arrow-right' : 'fa-arrow-left';
                        $status_color = [
                            'issued' => '#f59e0b',
                            'returned' => '#10b981',
                            'overdue' => '#ef4444',
                        ...
                        echo "<div class='activity-item {$status_class}'>
                                <div class='activity-icon' style='background-color: {$status_color}'>
                                    <i class='fas {$icon}'></i>
                                </div>
                                <div class='activity-details'>
                                    <div class='activity-main'>
                                        <strong>{$activity['title']}</strong> ({$activity['accession_number']})
                                    </div>
                                    <div class='activity-sub'>
                                        {$activity['full_name']} ({$activity['unique_identifier']})
                                    </div>
                                    <div class='activity-meta'>
                                        <span class='status-badge {$status_class}'>{$activity['display_status']}</span>
                ...
                echo '<div class="error-message">Error loading recent activity</div>';
            }
            ?>
        </div>
    </div>
